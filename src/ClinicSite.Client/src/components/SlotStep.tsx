...
import { getAllSlots, reserveSlot } from '../api/slots';
...
export function SlotStep({ specialty, doctor, onSelect }: Props) {
  const [slots, setSlots] = useState<AppointmentSlotDto[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [reservingId, setReservingId] = useState<string | null>(null);
  const [reserveError, setReserveError] = useState<string | null>(null);
  // Slots reserved in this session — marked immediately without waiting for reload
  const [justReservedIds, setJustReservedIds] = useState<Set<string>>(new Set());

  const load = () => {
    setLoading(true);
    setError(null);
    getAllSlots(doctor.id)
      .then(setSlots)
  ...
  const isUnavailable = (slot: AppointmentSlotDto) =>
    slot.status !== SlotStatus.Free || justReservedIds.has(slot.slotId);

  const handleSlotClick = (slot: AppointmentSlotDto) => {
    if (reservingId || isUnavailable(slot)) return;
    setReserveError(null);
    setReservingId(slot.slotId);
    reserveSlot(slot.slotId)
      .then((reservation) => {
        setJustReservedIds((prev) => new Set([...prev, slot.slotId]));
        onSelect(slot, reservation);
      })
      .catch((e: unknown) => {
        // Mark as unavailable locally if slot was taken
        setJustReservedIds((prev) => new Set([...prev, slot.slotId]));
  ...
  if (loading) return <Spinner text="Loading slots..." />;
  ...
  if (slots.length === 0) {
    return (
      <div className="empty-state">
        ...
        <p className="empty-title">No slots</p>
        <p className="empty-text">{doctor.fullName} has no scheduled appointments</p>
      </div>
    );
  }
